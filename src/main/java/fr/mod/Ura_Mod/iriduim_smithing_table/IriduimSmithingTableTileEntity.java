...
public class IriduimSmithingTableTileEntity extends TileEntity implements IInventory {
    ...
    public String getInventoryName() { //J'ai décider qu'on ne pouvait pas mettre de nom custom
        return "tile.contnaier_iriduim_smithing_table";
    }
    ...
    public boolean canSmeltItem() {
        if (this.contents[0] != null && this.contents[1] != null) {
            return IriduimSmithingTableRecipes.smelting().getSmeltingResult(new ItemStack[]{this.contents[0], this.contents[1]}) != null;
        }
        return false;
    }
    ...
    private boolean canSmelt()
    {
        ...
        else
        {
            ...
            ItemStack itemstack = IriduimSmithingTableRecipes.smelting().getSmeltingResult(new ItemStack[]{this.contents[0], this.contents[1]}); //Il y a une erreur ici, c'est normal, on y vient après (c'est pour les recettes)
    ...
    {
        if(canSmelt()){
            this.contents[2] = IriduimSmithingTableRecipes.smelting().getSmeltingResult(new ItemStack[]{this.contents[0], this.contents[1]});
        }
    }

    public boolean canCraft(ItemStack itemStack,ItemStack itemStack2){
        ItemStack itemstack = IriduimSmithingTableRecipes.smelting().getSmeltingResult(new ItemStack[]{itemStack, itemStack2}); //Il y a une erreur ici, c'est normal, on y vient après (c'est pour les recettes)
