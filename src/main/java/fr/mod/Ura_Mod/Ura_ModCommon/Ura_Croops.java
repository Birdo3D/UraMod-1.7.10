...
public class Ura_Croops extends BlockCrops {
    private IIcon[] field_149869_a;
    protected Item func_149865_P() {
        double p = Math.random() * ( 42 - 1 );
        int v = (int)p;
        System.out.println(v);
        if (v == 5) {
            return Ura_ModMain.iridium_potato;//iridium_nugget normalement
        }else {
            return Ura_ModMain.iridium_potato;
        }
    }
    ...
    protected Item func_149866_i(){double d = Math.random() * ( 14 - 1 );
        ...
        if (value == 5) {
            return Ura_ModMain.iridium_seed;
        }else {
            return null;
        }
    }
