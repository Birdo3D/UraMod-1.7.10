...
public class ChorusFlower extends Block {
    ...
    public ChorusFlower() {
        ...
        setBlockName("chrous_flower");
        setBlockTextureName(Ura_ModMain.MODID + ":chrous_flower");
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if (!canBlockStay(world, x, y, z))
            ...
            if (lowerBlock == Blocks.end_stone)
                canGrowUp = true;
            else if (lowerBlock == Ura_ModMain.chorus_plant) {
                int par8 = 1;
                int height;
                for (height = 0; height < 4; height++) {
                    Block b = world.getBlock(x, y - (par8 + 1), z);
                    if (b != Ura_ModMain.chorus_plant) {
            ...
            if (canGrowUp && isSpaceAroundFree(world, x, y + 1, z, ForgeDirection.DOWN) && world.isAirBlock(x, y + 2, z)) {
                world.setBlock(x, y, z, Ura_ModMain.chorus_plant);
                ...
                if (grew)
                    world.setBlock(x, y, z, Ura_ModMain.chorus_plant, 0, 3);
                else
